...
export default function Majors() {
  ...
  const fetchData = async () => {
    try {
      ...
      setError("Gagal memuat data jurusan. Silakan coba lagi nanti.");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const isDuplicate = majors.some(
        (m) =>
          m.name.toLowerCase() === namaJurusan.toLowerCase() &&
          (!isEditing || m.id !== editId)
      );
      if (isDuplicate) {
        setError("Nama jurusan sudah ada!");
        return;
      }

      if (isEditing) {
        await API.put(`/majors/${editId}`, { name: namaJurusan });
        setIsEditing(false);
        setEditId(null);
        showToast("Jurusan berhasil diperbarui!");
      } else {
        await API.post("/majors", { name: namaJurusan });
      ...
      setError(err.response?.data?.message || "Gagal menyimpan data jurusan!");
    }
  };
  ...
  const handleDelete = async () => {
    try {
      await API.delete(`/majors/${deleteId}`);
      showToast("Jurusan berhasil dihapus!");
      setShowModal(false);
      fetchData();
    } catch (err) {
      console.error("Gagal hapus jurusan:", err);
      setError("Gagal menghapus data jurusan. Coba lagi nanti!");
    }
  };
  ...
  return (
    <div className="min-h-screen text-slate-100 overflow-x-hidden mb-12 sm:mb-0 relative">
      ...
      <h2 className="text-3xl font-bold text-sky-400 mb-6">Data Jurusan</h2>
          ...
          <table className="min-w-full text-sm text-left border-collapse">
            <thead className="bg-sky-600/80 text-white">
              <tr>
                ...
                <th className="py-2 px-4 truncate">Nama Jurusan</th>
                <th className="py-2 px-4 truncate">Singkatan</th>
              ...
              {majors.length > 0 ? (
                majors.map((m, index) => (
                  ...
                  >
                    ...
                    <td className="py-2 px-4 truncate">{m.name}</td>
