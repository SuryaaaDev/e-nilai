...
export default function Teachers() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setError("");
      if (!nip || !namaGuru || !emailGuru || (!password && !isEditing)) {
  ...
  const handleDelete = async () => {
    try {
      ...
      setError("Gagal menghapus data guru. Coba lagi nanti!");
    }
  };
  ...
  return (
    <div className="min-h-screen text-slate-100 overflow-x-hidden mb-12 sm:mb-0 relative">
      ...
      <h2 className="text-3xl font-bold text-sky-400 mb-6">Data Guru</h2>
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-6 shadow-lg backdrop-blur-xl max-w-3xl mx-auto mb-8">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              const capitalized = value
                .replace(/([.?!])(\S)/g, "$1 $2")
                .toLowerCase()
                .replace(/(^\s*\w|[.?!]\s*\w|\s\w)/g, (match) =>
                  match.toUpperCase()
                );

              setNamaGuru(capitalized);
            }}
            className="bg-slate-800 border border-white/10 text-slate-100 px-4 py-2 rounded-xl focus:border-sky-400 focus:ring-2 focus:ring-sky-400 outline-none"
            required
          />
              ...
              {teachers.length > 0 ? (
                teachers.map((t, index) => (
                  ...
                  >
                    ...
                    <td className="py-2 px-4">{t.nip}</td>
                    <td className="py-2 px-4">{t.name}</td>
                    <td className="py-2 px-4 text-sky-300 font-semibold">
                      {t.user?.email}
                    </td>
