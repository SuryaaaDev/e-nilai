...
export default function Kelas() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        await API.put(`/classes/${editId}`, { name: namaKelas });
        setIsEditing(false);
        setEditId(null);
        showToast("Kelas berhasil diperbarui!");
      } else {
        await API.post("/classes", { name: namaKelas });
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-6 shadow-lg backdrop-blur-xl max-w-lg mx-auto mb-8">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <input
            ...
            value={namaKelas}
            onChange={(e) => setNamaKelas(e.target.value.toUpperCase())}
          ...
          <table className="min-w-full text-sm text-left border-collapse">
            <thead className="bg-sky-600/80 text-white">
              <tr>
                ...
                <th className="py-2 px-4 truncate">Nama Kelas</th>
              ...
              {kelas.length > 0 ? (
                kelas.map((k, index) => (
                  ...
                  >
                    ...
                    <td className="py-2 px-4 truncate">{k.name}</td>
